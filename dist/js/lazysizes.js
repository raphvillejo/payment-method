...
;(function (window, factory) {
  ...
  var loader = (function () {
            ...
            (preloadElems[0] ||
              (!elemExpandVal &&
                (eLbottom ||
                  ...
                  eLleft ||
                  eLtop ||
                  lazyloadElems[i].getAttribute(lazySizesConfig.sizesAttr) !=
                    "auto")))
          ) {
    ...
    var unveilElement = function (elem) {
      ...
      rafBatch(function lazyUnveil() {
        ...
        if (
          !(event = triggerEvent(elem, "lazybeforeunveil")).defaultPrevented
        ) {
          if (sizes) {
            if (isAuto) {
  ...
  ;(function () {
    ...
    var lazySizesDefaults = {
      ...
      hFac: 0.8,
      expand:
        docElem.clientHeight > 600
          ? docElem.clientWidth > 860
            ? 500
            : 410
          : 359,
      loadMode: 2,
...
// var lazysizes = (function($, document) {

// 	var evt = [

// 		// lazy before unveil - function that runs before an image is unveiled

// 		function($, document) {

// 			document.addEventListener('lazybeforeunveil', function(e){

// 				var parseSrcset = function(srcset) {
...
// 						if (arr.length === 1){ obj[0] = arr[0]; } else { obj[parseInt(arr[1], 10)] = arr[0]; }
// 					}

// 					return obj;
// 				};

// 				if ($(e.target).closest('figure').hasClass('cover-graphic') || $(e.target).closest('figure').hasClass('cover-photo')) {

// 					var target = $(e.target);
// 					var parent = target.closest('figure');
// 					var srcset = parseSrcset(e.target.getAttribute('data-srcset'));
// 					var retina = window.devicePixelRatio >= 1.5;
// 					var bg = srcset[window.devicePixelRatio] === undefined ? srcset[retina ? 2 : 1] : srcset[window.devicePixelRatio];

// 					parent.css('background-image', 'url("' + bg + '")');
// 				}
// 				else if ($(e.target).closest('figure').hasClass('unveil')) {

// 					setTimeout(function() { $(e.target).closest('figure').addClass('unveiled'); }, 1200);
// 				}
// 			});
// 		},
